...
class Attention(nn.Module):
    def __init__(self,
        ...
        if self.apply_transform:
            self.transform_conv1 = nn.Conv2d(self.num_heads, self.num_heads*4, kernel_size=1, stride=1)
            self.transform_act = nn.ReLU(inplace=True)
            self.transform_norm1 = nn.BatchNorm2d(self.num_heads*4)
            self.transform_conv2 = nn.Conv2d(self.num_heads*4, self.num_heads, kernel_size=1, stride=1)
...
class Block(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        #self.mlp = GCA_single(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)
    def forward(self, x, H, W):
