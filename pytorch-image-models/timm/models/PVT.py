...
from timm.models.layers import DropPath, to_2tuple, trunc_normal_, GCA
...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1, linear=False):
        ...
        self.transform_conv1 = nn.Conv2d(self.num_heads, self.num_heads*8, kernel_size=1, stride=1)
        self.transform_norm1 = nn.BatchNorm2d(self.num_heads*8)
        self.transform_conv2 = nn.Conv2d(self.num_heads*8, self.num_heads, kernel_size=1, stride=1)
    ...
    def forward(self, x, H, W):
        ...
        #attn = self.transform_conv1(attn)
        #attn = self.transform_norm1(attn)
        #attn = self.transform_conv2(attn)
...
class Attention(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        attn = (q @ k.transpose(-2, -1)) * self.scale
        #attn = torch.cdist(q, k, p = 2) * self.scale
        attn = attn.softmax(dim=-1)
        attn = self.attn_drop(attn)
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        #self.mlp = GCA_single(in_features=dim, act_layer=act_layer, drop=drop)
...
def pvt_small(pretrained=False, **kwargs):
    model = PyramidVisionTransformer(
        patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[8, 8, 4, 4], qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[2, 2, 6, 2], sr_ratios=[8, 4, 2, 1], **kwargs)
    model.default_cfg = _cfg()
