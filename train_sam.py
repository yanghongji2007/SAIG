...
from timm.models.vision_transformer_ import vit_small_patch16_224, resize_pos_embed
...
def setup(args):
    ...
    #print(model_grd)
    #state_dict = torch.load(os.path.join(args.pretrained_dir,'22Layer.pth.tar'))
...
def train(args, model):
    ...
    optimizer = SAM(model.parameters(), base_opt, 2.0, True, lr=args.learning_rate, weight_decay=args.weight_decay)
    ...
    for epoch in range(start_epoch, args.total_epoch):
        ...
        for step, (x_grd, x_sat) in enumerate(epoch_iterator):
            ...
            grd_global_2, sat_global_2 = model(x_grd,x_sat)
            criterion(grd_global_2, sat_global_2, args).backward()
